                    ...
                    <div class="flex home-house_ev" itemscope itemtype="http://schema.org/Person">
                        <section class="profil_info">
                            <div class="main-message__profil profil_pp">
                                ...
                                @else
                                        {{ 'https://eu.ui-avatars.com/api/?name=' . urlencode($user->name) . '&size=120&background=9165DF&color=ffffff' }} @endif
                            ...
                            <div class="flex edit__p">
                                <h2 class="profil__user-name {{ $user->house }}_c" itemprop="name">
                                    @if ($user->is_admin)
                                        <span class="tooltip">
                                            <img src="{{ asset('img/admin.svg') }}" alt="admin" title="admin"
                                                class="tag admin">
                                    ...
                                    @elseif ($user->is_modo)
                                        <span class="tooltip">
                                            <img src="{{ asset('img/modo.svg') }}" alt="modo" title="modo"
                                                class="tag modo">
                                            <span class="tooltiptext">
                                                modo
                                            </span>
                                    ...
                                    <div class="login_content">
                                        ...
                                        <div class="main-sec registration-sec notification_box">
                                            <form action="/profil/edit" method="post" enctype="multipart/form-data">
                                                ...
                                                <input type="file" name="profil" id="pp" class="file_input"
                                                ...
                                                <button
                                                    class="cta cta_i cta_l {{ Auth::user()->house }}_bg">Send</button>
                                            </form>
                                        </div>
                                    </div>
                                @else
                                    @if ($user->isFollowedByMe)
                                        <form action="{{ $user->id }}/rem_f" method="post" class="add_f">
                                            ...
                                            <button><img src="{{ asset('img/remove_f.svg') }}" alt=""
                                                    class="lm_img"><img src="{{ asset('img/d_remove.svg') }}"
                                                    alt="" class="dm_img"></button>
                                        </form>
                                    @else
                                            ...
                                            <button>
                                                @if ($user->house === 'ravenclaw')
                                                    <img src="{{ asset('img/add_f_r.svg') }}" alt=""
                                                        class="lm_img">
                                                    <img src="{{ asset('img/d_r_add.svg') }}" alt=""
                                                        class="dm_img">
                                                @endif
                                                @if ($user->house === 'gryffindor')
                                                    <img src="{{ asset('img/add_f_g.svg') }}" alt=""
                                                        class="lm_img">
                                                    <img src="{{ asset('img/d_g_add.svg') }}" alt=""
                                                        class="dm_img">
                                                @endif
                                                @if ($user->house === 'slytherin')
                                                    <img src="{{ asset('img/aff_f_s.svg') }}" alt=""
                                                        class="lm_img">
                                                    <img src="{{ asset('img/d_s_add.svg') }}" alt=""
                                                        class="dm_img">
                                                @endif
                                                @if ($user->house === 'hufflepuff')
                                                    <img src="{{ asset('img/add_f_h.svg') }}" alt=""
                                                        class="lm_img">
                                                    <img src="{{ asset('img/d_h_add.svg') }}" alt=""
                                                        class="dm_img">
                                                @endif
                                            </button>
                                        </form>
                            ...
                            <div class="flex profil__intro">
                                ...
                                <div class="profil__mehp">
                                    <div class="messages-house__profil flex">
                                        ...
                                        <div class="main-message__messages">
                                            <p class="messages__numbers {{ $user->house }}_c">{{ $user->house_point }}
                                            </p>
                                    ...
                                    <div class="profil__messages">
                                        <div class="lable_desc_user">About {{ $user->name }}</div>
                                        @if ($user->id === auth()->id())
                                            <form action="/profil/{{ $user->id }}/description" method="post"
                                                class="add_f desc_f">
                                                @csrf
                                                <textarea name="desc" id="desc_user" cols="30" rows="10" class="desc_text"
                                                    placeholder="Discribe yourself to the other users">{{ $user->description }}</textarea>
                                                <button class="cta cta_l update_btn">Update</button>
                                            </form>
                                        @else
                                            <p class="desc_texte__p" itemprop="description">"{{ $user->description }}"
                                            </p>
                                        @endif
                                    </div>
                ...
                <aside class="qeel qeel_profil">
                    ...
                    <section class="online">
                        ...
                        <div class="flex">
                            @foreach ($follows as $follow)
                                <a href="" href="/profil/{{ $follow->following->id }}" class="pp_link">
                                    <div class="flex pp__box-qeel">
                                        <div class="sm-pp__box {{ $follow->following->house }}_bg">
                                            <img src="@if ($follow->following->img) /{{ $follow->following->img }}
                                        @else
                                                {{ 'https://eu.ui-avatars.com/api/?name=' . urlencode($follow->following->name) . '&size=120&background=9165DF&color=ffffff' }} @endif"
                                                alt="" class="sm-pp">
                                        </div>
                                        <div class="name">
                                            @if ($follow->following->is_admin)
                                                <span class="tooltip">
                                                    <img src="{{ asset('img/admin.svg') }}" alt="admin" title="admin"
                                                        class="tag admin">
                                                    <span class="tooltiptext">
                                            ...
                                            @elseif ($follow->following->is_modo)
                                                <span class="tooltip">
                                                    <img src="{{ asset('img/modo.svg') }}" alt="modo" title="modo"
                                                        class="tag modo">
                                                    <span class="tooltiptext">
                            ...
                            @if ($follows->isEmpty())
                                <div class="">
                                    <p class="">You don't have any friends yet</p>
                                </div>
                            @endif
                    ...
                    <section class="">
                        ...
                        <div class="last-event__flex-contener">
                            @foreach ($parts as $part)
                                <a href="#">
                                    <section class="sm-event">
                                        <h4 class="sm-event__title">{{ $part->event->name }}</h4>
                                        ...
                                        <span class="sm-event__date__place"><img src="/img/where.svg" width="11" height="13"
                                                class="lm_img" alt="place"><img src="/img/d_place.svg" width="11"
                                                height="13" class="dm_img"
                                                alt="place">{{ $part->event->place }}</span>
                                    </section>
                                </a>
                            @endforeach
                            ...
                            @if ($parts->isEmpty())
                                <div class="">
                                    <p class="">You havent joined any event</p>
                                </div>
                            @endif
                ...
                <section class="last-topics">
                    ...
                    @foreach ($posts as $post)
                        <section class="topic-preview annim_home annimation_home">
                            <h3 class="lt__title under"><a
                                    href="{{ $post->postable->categorie->forum->slug }}/{{ $post->postable->categorie->slug }}/{{ $post->postable->slug }}">
                                    {{ $post->postable->title }}</a></h3>
                            <span class="lt__ariane under"> <a
                                    href="/{{ $post->postable->categorie->forum->slug }}">{{ $post->postable->categorie->forum->name }}</a>
                                ↣ <a
                                    href="/{{ $post->postable->categorie->forum->slug }}/{{ $post->postable->categorie->slug }}">
                                    {{ $post->postable->categorie->name }} </a> </span>
                    ...
                    @if ($posts->isEmpty())
                        <div class="nothing_box">
                            <img class="nothing_img lm_img" src="{{ asset('img/nothing.svg') }}" alt=""><img
                                class="nothing_img dm_img" src="{{ asset('img/d_nothing.svg') }}" alt="">

    ...
    <main class="main-sec">
        ...
        <section class="topic_box">
            ...
            <span class="lt__ariane under">
                <a href="/{{ $forum->slug }}">{{ $forum->name }} </a> ↣ <a
                    href="/{{ $forum->slug }}/{{ $categorie->slug }}">{{ $categorie->name }}</a> ↣ {{ $topic->title }}
            </span>
                    ...
                    <div class="main-message__text">
                        ...
                        <div class="edit-btn__topic">
                            @if ($post->likes->contains(auth()->id()))
                                <form class="message-like"
                                    action="/{{ $forum->slug }}/{{ $categorie->slug }}/{{ $topic->slug }}/unlike"
                                    method="post">
                                    @csrf
                                    @method('delete')
                                    <input type="hidden" name="post_id" value="{{ $post->id }}">
                                    <button><img src="{{ asset('img/liked.svg') }}" class="lm_img"
                                            alt="like the message"><img src="{{ asset('img/d_like.svg') }}"
                                            class="dm_img" alt="like the message"></button><br>
                                    <span class="like_c">{{ $post->likes->count() }}</span>
                                </form>
                            @else
                                <form class="message-like"
                                    action="/{{ $forum->slug }}/{{ $categorie->slug }}/{{ $topic->slug }}/like"
                                    method="post">
                                    @csrf
                                    <input type="hidden" name="post_id" value="{{ $post->id }}">
                                    <button><img src="{{ asset('img/not_like.svg') }}" class="lm_img"
                                            alt="Unlike the message"><img src="{{ asset('img/d_unlike.svg') }}"
                                            class="dm_img" alt="Unlike the message"></button><br>
                                    <span class="like_c">{{ $post->likes->count() }} </span>

                                </form>
                            @endif

                            <a href=""><img class="icone lm_img" src="{{ asset('img/quote.svg') }}"
                                    alt="edit the topic"><img class="icone dm_img" src="{{ asset('img/d_quote.svg') }}"
                                    alt="edit the topic" class="dm_img"></a>
                            @if ($post->user->id === auth()->id() || auth()->user()->is_modo)
                                ...
                                <label href="" class="in" for="del"><img class="icone lm_img"
                                        ...
                                        src="{{ asset('img/d_delete.svg') }}" alt="delete topic"></label>
                                <div class="not_box">
                                    ...
                                    <section class="main-sec registration-sec notification_box">
                                        ...
                                        <p>Are you sure you want to delete {{ $topic->title }} ? There is no spell to
                                            retore it once it's gone!</p>
                                        <div class="flex">
                                            <form action="" method="post" enctype="multipart/form-data">
                                                @csrf
                                                <button class="cta cta_i cta_l cta_not">Delete</button>
                                            </form>
                                            <div class="cta cta_l cta_not"><a
                                                    href="/{{ $forum->slug }}/{{ $categorie->slug }}/{{ $topic->slug }}">Back</a>
                                            </div>
                                        </div>
                ...
                <div class="message__box flex">
                    <div class="message__border">
                        <textarea name="body" id="" cols="30" rows="10"
                            placeholder="Type your new message here!">{{ old('body') }}</textarea>
                        <div class="mb_box">

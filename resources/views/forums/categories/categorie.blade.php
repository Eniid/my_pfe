                    ...
                    <section class="topic-list">
                        ...
                        <div class="sm-pp__box {{ $topic->user->house }}_bg">
                            <a href="/profil/{{ $topic->user->id }}" class="xxx profil">
                                <img src="
                                @if ($topic->user->img) /{{ $topic->user->img }}
                            @else
                                    {{ 'https://eu.ui-avatars.com/api/?name=' . urlencode($topic->user->name) . '&size=120&background=9165DF&color=ffffff' }} @endif"
                                    alt="" class="sm-pp">
                            </a>
                        </div>
                        <div class="topic-main_box">
                            <div class="flex">
                                <h3 class="under"><a
                                        href="/{{ $forum->slug }}/{{ $categorie->slug }}/{{ $topic->slug }}">{{ $topic->title }}</a>
                                </h3>

                            </div>
                            <p class="topix-preview__author__name">by <span><a href="#"
                                        class="{{ $topic->user->house }}_c">{{ $topic->user->name }}</a></span></p>
                        ...
                        <div class="latest-message">
                            <p class="topix-preview__author__name">latest message by <span><a href="#"
                                        class="{{ $topic->latestPost->user->house }}_c">{{ $topic->latestPost ? $topic->latestPost->user->name : '' }}</a></span>
                            </p>
                            <p class="topix-preview__author__messages">
                                {{ $topic->latestPost ? $topic->latestPost->created_at->diffForHumans() : '' }}</p>
                                    ...
                                    <section class="main-sec registration-sec notification_box">
                                        ...
                                        <div class="flex">
                                            ...
                                            <div class="cta cta_l cta_not"><a
                                                    href="/{{ $forum->slug }}/{{ $categorie->slug }}">Back</a></div>
                                        </div>

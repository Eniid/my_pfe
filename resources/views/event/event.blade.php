...
@section('content')
    ...
    <main class="main-sec single_event">
        <div class="frame">
            ...
            <section class="home-info">
                <div class="flex event_discribe">
                    <div class="event_basic">
                        ...
                        <p class="event_by">Created by <span class="under"><a href=""
                                    class="{{ $event->user->house }}_c">{{ $event->user->name }}</a></span></p>
                    ...
                    <div class="event_important">
                        <div class="sm-event__date">
                            <span class="sm-event__date__month">{{ $event->date->monthName }} {{ $event->date->day }}
                                - {{ $event->date->format('g.iA') }}</span>
                        </div>
                        ...
                        <span class="sm-event__date__place"><img src="/img/where.svg" width="11" height="13"
                                ...
                                class="dm_img" alt="place">{{ $event->place }}</span>
                        ...
                        @foreach ($event->event_user as $participant)
                            <a href="/profil/{{ $participant->user->id }}">
                                <div class="sm-pp__box {{ $participant->user->house }}_bg">
                                    <img src="
                                @if ($participant->user->img) /{{ $participant->user->img }}
                            @else
                                    {{ 'https://eu.ui-avatars.com/api/?name=' . urlencode($participant->user->name) . '&size=120&background=9165DF&color=ffffff' }} @endif"
                                        alt="" class="sm-pp">
                                ...
                                @foreach ($participations as $part)
                                    <a href="events/{{ $part->event->slug }}">
                                        <section class="sm-event">
                                            ...
                                            <div class="sm-event__date">
                                                <span
                                                    class="sm-event__date__month">{{ $part->event->date->monthName }}</span>
                                                <span class="sm-event__date__day">{{ $part->event->date->day }}</span>
                                                <span
                                                    class="sm-event__date__time">{{ $part->event->date->format('g.iA') }}</span>
                                            </div>
                                            <span class="sm-event__date__place"><img src="/img/where.svg" width="11"
                                                    height="13" class="lm_img" alt="place"><img
                                                    src="/img/d_place.svg" width="11" height="13" class="dm_img"
                                ...
                                <a href="events/{{ $event->slug }}">
                                    <section class="sm-event">
                                        ...
                                        <span class="sm-event__date__place"><img src="/img/where.svg" width="11" height="13"
                                                ...
                                                height="13" class="dm_img" alt="place">{{ $event->place }}</span>
                                    </section>
                ...
                @foreach ($event->posts as $post)
                    <div class="main-message flex">
                        ...
                        <div class="main-message__text">
                            ...
                            <div class="topix-preview__author flex">
                                ...
                                <div class="sm-pp__box {{ $post->user->house }}_bg">
                                    <a href="/profil/{{ $post->user->id }}">
                                        <img src="@if ($post->user->img) /{{ $post->user->img }}
                            @else
                                    {{ 'https://eu.ui-avatars.com/api/?name=' . urlencode($post->user->name) . '&size=120&background=9165DF&color=ffffff' }} @endif"
                                            alt="" class="sm-pp">
                                    </a>
                                </div>
                            </div>
                            ...
                            <div>
                                <span class="topix-preview__author__name under {{ $post->user->house }}_c"><span><a
                                            href="/profil/{{ $post->user->id }}">{{ $post->user->name }}</a></span> :
                                </span> {{ $post->body }}
                            ...
                            <div class="edit-btn__topic">
                                @if ($post->user->id === auth()->id() || auth()->user()->is_admin)
                                    ...
                                    <a href=""><img class="icone lm_img" src="{{ asset('img/edit.svg') }}"
                                            ...
                                            ></a><a href=""><img class="icone lm_img"
                                            src="{{ asset('img/delete.svg') }}" alt="delete topic"><img
                                            class="icone dm_img" src="{{ asset('img/d_delete.svg') }}"
                                            alt="delete topic"></a>
                                @endif
                            </div>
                        </div>
